...
public class HomeQueryTests
{
    ...
    public async void WhenUserSessionNotExist_ThrowNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => GetResponseFromHandler(testHelper, seed.userId, Guid.NewGuid()));
    }

    private static Task<ResponseDto<HomeResponseDto>> GetResponseFromHandler(CoreTestHelper coreTestHelper, Guid userId, Guid sessionId)
    {
